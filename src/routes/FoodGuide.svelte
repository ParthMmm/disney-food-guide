<script lang="ts">
    ...
    const filteredItems = $derived.by(() => {
        return items.filter((item) => {
            ...
            if (filtersStore.selectedTags.size > 0) {
                const hasAllTags = Array.from(filtersStore.selectedTags).every(
            ...
            if (filtersStore.selectedDates.size > 0) {
                ...
                const matchesAnyDate = Array.from(
                    filtersStore.selectedDates,
                ).some((dateFilter) => {
                    const cutoffDate =
    ...
    const activeFilterCount = $derived.by(() => {
        ...
        if (
            filtersStore.priceRange[0] !== 1 ||
            filtersStore.priceRange[1] !== 30
        )
            count++;
